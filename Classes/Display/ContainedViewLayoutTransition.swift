...
public extension ContainedViewLayoutTransition {
    ...
    func updateAlphaAndFrame(node: ASDisplayNode, alpha: CGFloat, frame: CGRect, beginWithCurrentState: Bool = false, force: Bool = false, delay: Double = 0.0, completion: ((Bool) -> Void)? = nil) {
        if node.alpha.isEqual(to: alpha) && !force {
            if let completion = completion {
                completion(true)
            }
